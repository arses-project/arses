...
#if defined(NDEBUG)
# error "Arses cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xb28ec3db0cdea1fe287f5803a50b31764e205d20c6943d7cb22f23c0319ce133");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Arses: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Arses Signed Message:\n";
...
{
    // Arses: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 800000); // Arses: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3 * 24 * 60 * 60; // Arses: 3 days
static const int64 nTargetSpacing = 4 * 60; // Arses: 4 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Arses: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Arses: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x7c03821cf5702a16153fb6fd2a4d265009f14d5c32dfa1a283bc271e20b74ff2");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "NY Times 01/April/2019 Iran Hit With 3rd Major Flood in 2 Weeks";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 800 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("0496ebaa2a062edf7b36f2acce2b8f1bc740f84c409de51fb3405387e7379a78436f54a8b427f0d67dd90db8be16264f4cbda92f63603645c02d6a6a33dc3c7b65") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1554285144;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085246101;

        if (fTestNet)
        {
            block.nTime    = 1554285121;
            block.nNonce   = 385985379;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
	 {
        ...
        assert(block.hashMerkleRoot == uint256("0xd1cd15213c607ae33367b12f1f2ef7b1f07a1e10617340e4d3a71f13e7d059de"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xd4, 0xba, 0xc7, 0xf2 }; // Arses: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("ArsesMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("ArsesMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ArsesMiner(CWallet *pwallet)
{
    printf("ArsesMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("arses-miner");
    ...
    try { loop {
        ...
        printf("Running ArsesMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&ArsesMiner, pwallet));
}
